...
template <class FIter1, class FIter2> void iter_swap(FIter1 a, FIter2 b) {
  mystl::swap(*a, *b);
}
...
OutputIter unchecked_copy_cat(RandomIter first, RandomIter last,
  ...
  for (auto n = last - first; n > 0; --n, ++first, (void)++result) {
    *result = *first;
  }
  return result;
}
...
/*********************************************************** */
// equal
//  比较第一序列在[first,last)区间上的元素是否和第二序列相等
/*********************************************************** */
...
// 2.如果到达last1而尚未到达last2,则返回true
// 3.如果到达last2而尚未到达last1,则返回false
// 如果同时到达last1和last2,则返回false
/*************************************************************** */
template <class InputIter1, class InputIter2>
